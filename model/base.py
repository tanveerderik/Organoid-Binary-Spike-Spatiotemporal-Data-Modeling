...
class ContinuousResidualProjector(nn.Module):
    """
    Project the frozen encoder residual onto the convex hull formed by the
    selected z1 parent's frozen z2 residual vectors.

    For an active token,

        r_e = z_e - z1
        r_proj = argmin_{r in conv(z2_1, ..., z2_K)} ||r - r_e||_2^2
        z_cont = z1 + r_proj

    Stage 2A and the default Stage 2C path use the deterministic geometric
    projection directly.  The optional alpha adapter is only an amortized
    approximation to that fixed projection; it does not define the geometry.

    No zero anchor or nonlinear radial squash is used. A fixed optional radial
    margin may scale all child residual vertices uniformly around z1 before the
    convex projection. The resulting feasible set remains a well-defined convex
    hull with simplex coordinates.
    """
    ...
    def __init__(
        ...
        sample_mix: float = 0.0,
        safe_fraction: float = 0.85,
        parent_margin_fraction: float = 0.10,
        hull_margin_fraction: float = 0.05,
        projection_steps: int = 64,
        eps: float = 1e-8,
    ):
        super().__init__()
        self.code_dim = int(code_dim)
        self.num_child_codes = int(num_child_codes)
        self.num_anchors = self.num_child_codes
        self.posterior_temperature = float(posterior_temperature)
        self.gumbel_temperature = float(gumbel_temperature)
        self.sample_mix = float(sample_mix)
        self.safe_fraction = float(safe_fraction)
        self.parent_margin_fraction = float(parent_margin_fraction)
        ...
        if self.projection_steps <= 0:
            raise ValueError(
                f"projection_steps must be positive, got {self.projection_steps}"
            )
        ...
        # Optional amortized projector.  Its zero-initialized final layer means
        # it begins from the distance-kernel reference logits.
        ...
        nn.init.zeros_(self.alpha_adapter[-1].weight)
        nn.init.zeros_(self.alpha_adapter[-1].bias)

        # Retained only for checkpoint compatibility with the earlier Stage-2
        # implementation.  It is deliberately not used: scaling a convex
        # combination around zero can leave the child convex hull.
        self.hull_scale_adapter = nn.Sequential(
            nn.LayerNorm(self.code_dim),
            nn.Linear(self.code_dim, hidden_dim),
            nn.GELU(),
            nn.Linear(hidden_dim, 1),
        )
        nn.init.zeros_(self.hull_scale_adapter[-1].weight)
        nn.init.zeros_(self.hull_scale_adapter[-1].bias)
        for parameter in self.hull_scale_adapter.parameters():
            parameter.requires_grad = False

        self.use_alpha_adapter = False

        # When True, decoding uses the exact geometric projection even if the
        # optional adapter is being trained.  This prevents reconstruction loss
        # from redefining the adapter's target.
        self.decode_with_projection_target = True

    def _safe_radii(self, z1_table: torch.Tensor) -> torch.Tensor:
        """Return a diagnostic half-nearest-neighbour radius per z1 centroid."""
        centers_f = z1_table.detach().float()
        if centers_f.shape[0] <= 1:
            return centers_f.new_full((centers_f.shape[0],), float("inf"))
        center_dist = torch.cdist(centers_f, centers_f, p=2)
        center_dist.fill_diagonal_(float("inf"))
        nearest = center_dist.min(dim=1).values
        return 0.5 * self.safe_fraction * nearest

    @staticmethod
    def _project_rows_to_simplex(
        values: torch.Tensor,
        eps: float,
    ) -> torch.Tensor:
        """Euclidean projection of each row onto {a >= 0, sum(a) = 1}."""
        ...
        theta = cumulative.gather(1, rho[:, None]) / (
            rho.to(values.dtype)[:, None] + 1.0
        )
    ...
    @torch.no_grad()
    def _project_onto_child_hull(
        self,
        target_residual: torch.Tensor,  # (M,D)
        children: torch.Tensor,         # (M,K,D)
        initial_alpha: Optional[torch.Tensor] = None,
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        Batched accelerated projected-gradient solution of

            min_alpha 0.5 ||sum_k alpha_k child_k - target||^2
            subject to alpha >= 0 and sum(alpha) = 1.

        The simplex projection enforces the convex-hull constraint at every
        iteration.  The step size uses a batched power-iteration estimate of
        the squared spectral norm of each local child matrix.
        """
        target_f = target_residual.detach().float()
        children_f = children.detach().float()
        M, K, _ = children_f.shape

        if initial_alpha is None:
            d2 = (
                target_f[:, None, :] - children_f
            ).pow(2).sum(dim=-1)
            alpha = F.softmax(-d2, dim=-1)
        else:
            alpha = initial_alpha.detach().float()
            alpha = self._project_rows_to_simplex(alpha, self.eps)

        # Estimate L = ||A||_2^2 for each token without materializing a KxK
        # Gram matrix.  A conservative 0.95/L step keeps PGD stable.
        power_vec = children_f.new_full(
            (M, K),
            1.0 / max(1.0, float(K) ** 0.5),
        )
        for _ in range(8):
            latent_vec = torch.einsum(
                "mk,mkd->md",
                power_vec,
                children_f,
            )
            gram_vec = torch.einsum(
                "md,mkd->mk",
                latent_vec,
                children_f,
            )
            power_vec = gram_vec / gram_vec.norm(
                dim=-1,
                keepdim=True,
            ).clamp_min(self.eps)
        ...
        step_size = (0.95 / lipschitz)[:, None]

        current = alpha
        extrapolated = alpha
        momentum = 1.0

        # Keep the best feasible point seen.  The nearest hard child and the
        # distance-kernel initialization are both feasible hull points, so the
        # finite-iteration solver is never allowed to return a worse geometric
        # approximation than either baseline.
        best_alpha = alpha.clone()
        best_point = torch.einsum(
            "mk,mkd->md",
            best_alpha,
            children_f,
        )
        best_objective = (best_point - target_f).pow(2).sum(dim=-1)

        hard_index = (
            target_f[:, None, :] - children_f
        ).pow(2).sum(dim=-1).argmin(dim=-1)
        hard_alpha = F.one_hot(
            hard_index,
            num_classes=K,
        ).to(dtype=children_f.dtype)
        hard_point = torch.einsum(
            "mk,mkd->md",
            hard_alpha,
            children_f,
        )
        hard_objective = (hard_point - target_f).pow(2).sum(dim=-1)
        use_hard = hard_objective < best_objective
        best_alpha = torch.where(
            use_hard[:, None],
            hard_alpha,
            best_alpha,
        )
        best_objective = torch.minimum(best_objective, hard_objective)
        ...
        for _ in range(self.projection_steps):
            ...
            updated = self._project_rows_to_simplex(
                extrapolated - step_size * gradient,
                self.eps,
            )

            updated_point = torch.einsum(
                "mk,mkd->md",
                updated,
                children_f,
            )
            updated_objective = (
                updated_point - target_f
            ).pow(2).sum(dim=-1)
            improved = updated_objective < best_objective
            best_alpha = torch.where(
                improved[:, None],
                updated,
                best_alpha,
            )
            best_objective = torch.minimum(
                best_objective,
                updated_objective,
            )

            next_momentum = 0.5 * (
                1.0 + (1.0 + 4.0 * momentum * momentum) ** 0.5
            )
        ...
        alpha = self._project_rows_to_simplex(best_alpha, self.eps)
        point = torch.einsum("mk,mkd->md", alpha, children_f)
        return alpha, point

    def forward(
        self,
        z_e_active: torch.Tensor,   # (M,D)
        z1_codes: torch.Tensor,     # (M,)
        z1_table: torch.Tensor,     # (K1,D)
        z2_table: torch.Tensor,     # (K1,K2,D)
        *,
        child_scale: float = 1.0,
        sample: bool = False,
        posterior_temperature: Optional[float] = None,
        gumbel_temperature: Optional[float] = None,
        sample_mix: Optional[float] = None,
    ):
        if z_e_active.ndim != 2:
            raise ValueError(
                f"z_e_active must be (M,D), got {tuple(z_e_active.shape)}"
            )
        if z1_codes.ndim != 1 or z1_codes.shape[0] != z_e_active.shape[0]:
            raise ValueError(
                f"z1_codes must be ({z_e_active.shape[0]},), "
                f"got {tuple(z1_codes.shape)}"
            )
        if z2_table.ndim != 3:
            raise ValueError(
                f"z2_table must be (K1,K2,D), got {tuple(z2_table.shape)}"
            )

        del sample, gumbel_temperature, sample_mix  # Gumbel is intentionally disabled.

        dtype = z_e_active.dtype
        device = z_e_active.device
        z1_codes = z1_codes.to(device=device, dtype=torch.long)

        z1_table_d = z1_table.detach().to(device=device, dtype=dtype)
        z2_table_d = z2_table.detach().to(device=device, dtype=dtype)

        centers = z1_table_d[z1_codes]                              # (M,D)
        
        # Expand the child convex hull radially around the selected z1 center.
        # A margin of 0.05 places each hull vertex 5% farther from z1.
        hull_scale = 1.0 + max(0.0, float(self.hull_margin_fraction))
        ...
        target_residual = z_e_active - centers                      # (M,D)

        d2 = (
            target_residual[:, None, :] - children
        ).float().pow(2).sum(dim=-1)                                # (M,K2)

        anchor_scale = children.float().pow(2).sum(dim=-1).mean(
            dim=-1,
            keepdim=True,
        ).clamp_min(self.eps)
        ...
        post_tau = float(
            ...
            if posterior_temperature is None
            else posterior_temperature
        )
        post_tau = max(post_tau, self.eps)
        posterior_logits = -d2 / (post_tau * anchor_scale)
        alpha_reference = F.softmax(
            posterior_logits,
            dim=-1,
        ).to(dtype=dtype)

        alpha_target_f, residual_target_f = self._project_onto_child_hull(
            target_residual=target_residual,
            children=children,
            initial_alpha=alpha_reference,
        )
        alpha_target = alpha_target_f.to(device=device, dtype=dtype)
        residual_target = residual_target_f.to(device=device, dtype=dtype)

        adapter_input = (
            target_residual.float()
            / anchor_scale.sqrt().clamp_min(self.eps)
        ).to(dtype=dtype)

        if self.use_alpha_adapter:
            adapter_delta_logits = self.alpha_adapter(
                adapter_input.detach()
            ).float()
            adapted_logits = posterior_logits.detach() + adapter_delta_logits
            alpha_pred = F.softmax(adapted_logits, dim=-1).to(dtype=dtype)
        else:
            adapter_delta_logits = posterior_logits.new_zeros(
                posterior_logits.shape
            )
            alpha_pred = alpha_target

        residual_pred = torch.einsum(
            "mk,mkd->md",
            alpha_pred,
            children,
        )

        use_projection_target = (
            not self.use_alpha_adapter
            or (self.decode_with_projection_target and self.training)
        )
        if use_projection_target:
            alpha_decode = alpha_target
            residual_decode = residual_target
        else:
            alpha_decode = alpha_pred
            residual_decode = residual_pred

        z_cont_active = centers + residual_decode

        # Stage 2B objective: make the optional adapter reproduce the fixed
        # geometric projection.  The decoder does not define this target.
        if self.use_alpha_adapter:
            ...
            projection_loss = z_e_active.new_zeros(())

        geometric_projection_mse = F.mse_loss(
            residual_target,
            target_residual.detach(),
        )
        distance_reference_residual = torch.einsum(
            "mk,mkd->md",
            alpha_reference,
            children,
        )
        distance_reference_mse = F.mse_loss(
            distance_reference_residual,
            target_residual.detach(),
        )
        hard_vertex_mse = (
            d2.min(dim=-1).values.mean()
            / max(1, target_residual.shape[-1])
        ).to(dtype=dtype)

        safe_radius_all = self._safe_radii(z1_table_d).to(
            device=device,
            dtype=dtype,
        )
        safe_radius = safe_radius_all[z1_codes]

        # Diagnostic parent assignment margin evaluated at the projected point.
        all_d2 = torch.cdist(
            z_cont_active.float(),
            z1_table_d.float(),
            p=2,
        ).pow(2)
        d_pos = all_d2.gather(1, z1_codes[:, None]).squeeze(1)
        all_d2_other = all_d2.clone()
        all_d2_other.scatter_(1, z1_codes[:, None], float("inf"))
        d_neg = all_d2_other.min(dim=1).values
        parent_margin_loss = F.relu(d_pos - d_neg).mean().to(dtype=dtype)

        def _mean_row_entropy(weights: torch.Tensor) -> torch.Tensor:
            weights_f = weights.float().clamp_min(self.eps)
            return -(weights_f * weights_f.log()).sum(dim=-1).mean()

        def _usage_entropy(weights: torch.Tensor) -> torch.Tensor:
            usage = weights.float().mean(dim=0).clamp_min(self.eps)
            usage = usage / usage.sum().clamp_min(self.eps)
            return -(usage * usage.log()).sum()

        entropy_target = _mean_row_entropy(alpha_target)
        entropy_pred = _mean_row_entropy(alpha_pred)
        one = z_e_active.new_ones((z_e_active.shape[0],))
        zero = z_e_active.new_zeros((z_e_active.shape[0],))

        aux = {
            "alpha": alpha_decode,
            "alpha_target": alpha_target,
            "alpha_pred": alpha_pred,
            "alpha_mean": alpha_reference,
            "alpha_sample": alpha_pred,
            "posterior_logits": posterior_logits,
            "target_residual": target_residual,
            "residual_hull": residual_decode,
            "residual_target": residual_target,
            "residual_pred": residual_pred,
            "residual_raw": residual_decode,
            "residual_safe": residual_decode,
            "hull_scale": one,
            "hull_scale_logit": zero,
            "mean_hull_scale": z_e_active.new_tensor(1.0),
            "mean_hull_scale_delta": z_e_active.new_zeros(()),
            "hull_expansion_fraction": z_e_active.new_zeros(()),
            "safe_radius": safe_radius,
            "projection_loss": projection_loss,
            "geometric_projection_mse": geometric_projection_mse,
            "distance_reference_mse": distance_reference_mse,
            "hard_vertex_mse": hard_vertex_mse,
            "projection_gain_vs_hard": hard_vertex_mse - geometric_projection_mse,
            "parent_margin_loss": parent_margin_loss,
            "alpha_entropy": entropy_target.to(dtype=dtype),
            "alpha_mean_entropy": _mean_row_entropy(alpha_reference).to(dtype=dtype),
            "alpha_sample_entropy": entropy_pred.to(dtype=dtype),
            "alpha_final_entropy": _mean_row_entropy(alpha_decode).to(dtype=dtype),
            "alpha_final_usage_entropy": _usage_entropy(alpha_decode).to(dtype=dtype),
            "alpha_mean_max": alpha_reference.float().max(dim=-1).values.mean().to(dtype=dtype),
            "alpha_sample_max": alpha_pred.float().max(dim=-1).values.mean().to(dtype=dtype),
            "alpha_final_max": alpha_decode.float().max(dim=-1).values.mean().to(dtype=dtype),
            "adapter_delta_logit_norm": adapter_delta_logits.float().norm(
                dim=-1
            ).mean().to(dtype=dtype),
            "mean_residual_norm": residual_decode.norm(dim=-1).mean(),
            "mean_safe_radius": safe_radius.mean(),
            "sample_mix": z_e_active.new_zeros(()),
            "alpha_adapter_enabled": z_e_active.new_tensor(
                float(self.use_alpha_adapter)
            ),
            "decode_uses_projection_target": z_e_active.new_tensor(
                float(use_projection_target)
            ),
        }
        return z_cont_active, aux
